...
var bindEventTodoDelete = function() {
    ...
    todoList.addEventListener('click', function(event){
        ...
        if(self.classList.contains('routes-delete')){
            // 删除这个 routes
...
var bindEventTodoEdit = function() {
    ...
    todoList.addEventListener('click', function(event){
        var self = event.target
        if(self.classList.contains('routes-edit')){
            // 删除这个 routes
            var todoCell = self.parentElement
...
var bindEventTodoUpdate = function() {
    ...
    todoList.addEventListener('click', function(event){
        var self = event.target
        if(self.classList.contains('routes-update')){
            log('点击了 update ')
            //
            var editForm = self.parentElement
            // querySelector 是 DOM 元素的方法
            // document.querySelector 中的 document 是所有元素的祖先元素
            var input = editForm.querySelector('.routes-edit-input')
            var title = input.value
            // 用 closest 方法可以找到最近的直系父节点
            var todoCell = self.closest('.routes-cell')
            var todo_id = todoCell.dataset.id
            var form = {
                'id': todo_id,
                'title': title,
            ...
            apiTodoUpdate(form, function(r){
                log('更新成功', todo_id)
                var todo = JSON.parse(r)
                var selector = '#routes-' + todo.id
                var todoCell = e(selector)
                var titleSpan = todoCell.querySelector('.routes-title')
