...
# 将复杂的逻辑分割开变成独立的小的逻辑
# 请求和处理响应的过程：
# parse url，根据 protocol, host, port, path 构建 request
# 使用 socket 发送相应（http, https）的请求
# 接收 response
# parse response 得到 status_code, headers, body
...
def parse_response(r):
    """
    把 response 解析出 status code, headers, body 返回
    status code : int
    headers : dict
    body : str
    """
